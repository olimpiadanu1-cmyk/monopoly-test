...
async function saveGameState() {
    ...
    // Сохраняем состояния игр всех пользователей на сервер
    const allGameStates = await gameAPI.loadData('game_states') || {};
    allGameStates[currentUser.id] = gameState;
    gameAPI.saveData('game_states', allGameStates);
}
...
async function saveLeaderboard(players) {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('leaderboard', players);
}
...
async function loadGameState() {
    ...
    const allGameStates = await gameAPI.loadData('game_states') || {};
    const serverGameState = allGameStates[currentUser.id];
    ...
    if (serverGameState) {
        ...
        // Если на сервере нет, пытаемся загрузить из localStorage
        const savedState = localStorage.getItem(`monopoly_game_state_${currentUser.id}`);
        if (savedState) {
            gameState = JSON.parse(savedState);
            console.log('⚠️ Состояние игры загружено из localStorage');
        }
    }
...
function registerUser() {
    ...
    // Сохраняем на сервер
    gameAPI.saveData('applications', applications);
...
async function submitTaskForReview() {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('task_submissions', submissions);
...
async function approveApplication(appId) {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('users', users);
    await gameAPI.saveData('applications', applications);
...
async function addShopItem() {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('shop_items', shopItems);
...
async function confirmPurchase() {
    ...
    // Сохраняем заявку на сервер
    await gameAPI.saveData('purchase_requests', purchaseRequests);
    ...
    await gameAPI.saveData('shopping_carts', allShoppingCarts);
...
async function updateCartCount() {
    ...
    try {
        const serverData = await gameAPI.loadData('purchase_requests');
        if (Array.isArray(serverData)) {
            purchaseRequests = serverData;
            localStorage.setItem('monopoly_purchase_requests', JSON.stringify(purchaseRequests));
        } else {
            throw new Error('Некорректный формат данных');
        }
    } catch (error) {
        console.error('Ошибка загрузки заявок на покупку для счетчика:', error);
...
async function showCart() {
    // Перезагружаем данные с сервера перед показом корзины
    try {
        const serverData = await gameAPI.loadData('purchase_requests');
        // Проверяем, что получили корректные данные
        if (Array.isArray(serverData)) {
            purchaseRequests = serverData;
            console.log('🔄 Данные заявок загружены с сервера:', purchaseRequests.length);
            console.log('📊 Пример данных:', purchaseRequests.slice(0, 2));
            localStorage.setItem('monopoly_purchase_requests', JSON.stringify(purchaseRequests));
        } else {
            console.error('❌ Некорректный формат данных с сервера:', serverData);
            throw new Error('Некорректный формат данных');
        }
    } catch (error) {
        ...
        // Fallback к localStorage
        purchaseRequests = JSON.parse(localStorage.getItem('monopoly_purchase_requests') || '[]');
        console.log('📱 Используем данные из localStorage:', purchaseRequests.length);
    }
    ...
    // Обновляем счетчик корзины после загрузки данных
    await updateCartCount();
}
...
async function showCartTab(tabName) {
    // Переключаем табы
    ...
    try {
        ...
        if (Array.isArray(serverData)) {
            purchaseRequests = serverData;
            localStorage.setItem('monopoly_purchase_requests', JSON.stringify(purchaseRequests));
            console.log('🔄 Обновлены данные при переключении вкладки:', tabName);
        } else {
            throw new Error('Некорректный формат данных');
        }
    } catch (error) {
        console.error('Ошибка обновления данных при переключении вкладки:', error);
        purchaseRequests = JSON.parse(localStorage.getItem('monopoly_purchase_requests') || '[]');
    }
...
function loadCartItems() {
    ...
    console.log('🛒 Загрузка корзины для пользователя:', currentUser.id);
    console.log('📦 Всего заявок пользователя:', userRequests.length);
    
    // Загружаем ожидающие
    const pendingItems = userRequests.filter(req => req.status === PURCHASE_STATUS.PENDING);
    console.log('⏳ Ожидающих:', pendingItems.length);
    ...
    console.log('✅ Одобренных:', approvedItems.length);
    ...
    console.log('❌ Отклоненных:', rejectedItems.length);
...
// Функция для отладки корзины
async function debugCart() {
    console.log('🔧 Отладка корзины...');
    console.log('👤 Текущий пользователь:', currentUser?.id, currentUser?.nick);
    
    // Загружаем свежие данные
    try {
        const freshData = await gameAPI.loadData('purchase_requests');
        console.log('📊 Тип полученных данных:', typeof freshData, Array.isArray(freshData));
        console.log('📊 Свежие данные с сервера:', freshData?.length || 'не массив');
        console.log('🔍 Сами данные:', freshData);
        
        if (currentUser && Array.isArray(freshData)) {
            const userRequests = freshData.filter(req => req.userId === currentUser.id);
            console.log('👤 Заявки пользователя:', userRequests.length);
            
            const pending = userRequests.filter(req => req.status === PURCHASE_STATUS.PENDING);
            const approved = userRequests.filter(req => req.status === PURCHASE_STATUS.APPROVED);
            const rejected = userRequests.filter(req => req.status === PURCHASE_STATUS.REJECTED);
            
            console.log('⏳ Ожидающих:', pending.length);
            console.log('✅ Одобренных:', approved.length);
            console.log('❌ Отклоненных:', rejected.length);
            
            console.log('📝 Детали заявок:', userRequests);
        }
    } catch (error) {
...
async function approveTask(submissionId) {
    ...
    userGameState.moves = (userGameState.moves || 30) + 1;
    localStorage.setItem(`monopoly_game_state_${submission.userId}`, JSON.stringify(userGameState));
    
    // Сохраняем на сервер
    await gameAPI.saveData('task_submissions', submissions);
    
    // Обновляем состояния игр на сервере
    const allGameStates = await gameAPI.loadData('game_states') || {};
    allGameStates[submission.userId] = userGameState;
    await gameAPI.saveData('game_states', allGameStates);
...
async function giveMoves() {
    ...
    // Сохраняем состояния игр на сервере
    const allGameStates = await gameAPI.loadData('game_states') || {};
    allGameStates[user.id] = gameState;
    await gameAPI.saveData('game_states', allGameStates);
    
    // Записываем в историю
    await addRewardHistory('moves', nickname, amount);
...
async function givePoints() {
    ...
    // Обновляем очки в массиве пользователей
    ...
    // Добавляем очки в состояние игры
    ...
    // Записываем в историю
    await addRewardHistory('points', nickname, amount);
...
async function addRewardHistory(type, nickname, amount) {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('reward_history', history);
}
...
async function saveAllTasks() {
    ...
    // Сохраняем на сервер
    await gameAPI.saveData('cell_tasks', cellTasks);
...
// Функции для работы с серверным API
async function loadDataFromServer() {
    try {
        console.log('🔄 Загрузка данных с сервера...');
        
        // Загружаем основные данные
        users = await gameAPI.loadData('users');
        applications = await gameAPI.loadData('applications');
        purchaseRequests = await gameAPI.loadData('purchase_requests');
        cellTasks = await gameAPI.loadData('cell_tasks');
        
        // Загружаем задания от игроков
        const taskSubmissions = await gameAPI.loadData('task_submissions');
        if (taskSubmissions && taskSubmissions.length > 0) {
            localStorage.setItem('monopoly_task_submissions', JSON.stringify(taskSubmissions));
        }
        
        // Загружаем историю выдачи наград
        const rewardHistory = await gameAPI.loadData('reward_history');
        if (rewardHistory && rewardHistory.length > 0) {
            localStorage.setItem('monopoly_reward_history', JSON.stringify(rewardHistory));
        }
        
        // Загружаем таблицу лидеров
        const leaderboard = await gameAPI.loadData('leaderboard');
        if (leaderboard && leaderboard.length > 0) {
            localStorage.setItem('monopoly_leaderboard', JSON.stringify(leaderboard));
        }
        
        // Загружаем товары в магазине
        const shopItemsData = await gameAPI.loadData('shop_items');
        ...
        const shoppingCarts = await gameAPI.loadData('shopping_carts');
        if (shoppingCarts && Object.keys(shoppingCarts).length > 0) {
            // Сохраняем корзины для каждого пользователя в localStorage
            Object.entries(shoppingCarts).forEach(([userId, cartData]) => {
                localStorage.setItem(`monopoly_shopping_cart_${userId}`, JSON.stringify(cartData));
            });
        }
        ...
        console.log('🛒 Заявок на покупку:', purchaseRequests.length);
        console.log('📋 Заданий от игроков:', taskSubmissions.length);
        console.log('🏆 Записей в истории наград:', rewardHistory.length);
        console.log('🏅 Игроков в лидерборде:', leaderboard.length);
        console.log('🛍️ Товаров в магазине:', shopItemsData.length);
        console.log('🛒 Корзин пользователей:', Object.keys(shoppingCarts).length);
...
async function saveDataToServer() {
    try {
        ...
        // Сохраняем задания и историю если они есть
        const taskSubmissions = JSON.parse(localStorage.getItem('monopoly_task_submissions') || '[]');
        if (taskSubmissions.length > 0) {
            await gameAPI.saveData('task_submissions', taskSubmissions);
        }
        
        const rewardHistory = JSON.parse(localStorage.getItem('monopoly_reward_history') || '[]');
        if (rewardHistory.length > 0) {
            await gameAPI.saveData('reward_history', rewardHistory);
        }
        
        const leaderboard = JSON.parse(localStorage.getItem('monopoly_leaderboard') || '[]');
        if (leaderboard.length > 0) {
            await gameAPI.saveData('leaderboard', leaderboard);
        }
        
        // Сохраняем товары в магазине
        if (shopItems && shopItems.length > 0) {
            await gameAPI.saveData('shop_items', shopItems);
        }
        
        // Сохраняем корзины пользователей
        const allShoppingCarts = {};
        // Собираем все корзины из localStorage
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key && key.startsWith('monopoly_shopping_cart_')) {
                const userId = key.replace('monopoly_shopping_cart_', '');
        ...
        if (Object.keys(allShoppingCarts).length > 0) {
            await gameAPI.saveData('shopping_carts', allShoppingCarts);
        }
...
// Функция handleImportFile удалена - используется автоматическое серверное хранилище
